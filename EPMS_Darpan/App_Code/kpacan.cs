...
public class kpacan
{
    ...
    public void Cancel(int finyr, string sail_pno, int kpa_slno, string kpa_desc, decimal max_marks, int dept_cd, int unit_cd, string kpa_type, string ins_upd, string cancel_rem)
    {
        OleDbCommand cmd = new OleDbCommand("cancel_kpa_mas", con);
        cmd.CommandType = CommandType.StoredProcedure;
        OleDbParameter p1 = new OleDbParameter("@finyr", OleDbType.Integer, 4, "finyr");
        OleDbParameter p2 = new OleDbParameter("@sail_pno", OleDbType.VarChar, 8, "sail_pno");
        OleDbParameter p3 = new OleDbParameter("@kpa_slno", OleDbType.Integer, 2, "kpa_slno");
        ...
        OleDbParameter p8 = new OleDbParameter("@kpa_type", OleDbType.Char, 1, "kpa_type");
        OleDbParameter p9 = new OleDbParameter("@ins_upd", OleDbType.Char, 1, "ins_upd");
        OleDbParameter p10 = new OleDbParameter("@cancel_rem", OleDbType.Char, 50, "cancel_rem");
    ...
    {
        string sql = "Select kpa_slno,kpa_desc,max_marks,decode(kpa_type,'R','ROUTINE','N','NON-ROUTINE','S','SPECIAL') kpa_type,cancel_rem,allow_ind from kpa_mas where finyr="+@finyear+" and sail_pno='"+@sail_pno+"' order by kpa_slno";
  ...
  //  public decimal maxmm(int finyear, string sail_pno)
    ...
    public bool activity_check(int finyear, string sail_pno)
    {
     // string sql = "Select count(1) from activity_det  where kpa_slno not in (select kpa_slno from kpa_mas  where sail_pno='" + sail_pno + "' and finyr=" + finyear + ") and sail_pno='" + sail_pno + "' and finyr=" + finyear;
       string sql = "Select count(1) from kpa_mas  where cancel_dt is null and kpa_slno not in (select distinct kpa_slno from activity_det  where sail_pno='" + sail_pno + "' and finyr=" + finyear + ") and sail_pno='" + sail_pno + "' and finyr=" + finyear;
        bool dtyesno =false;
        int dt;
        OleDbCommand cmd = new OleDbCommand(sql, con);
        OleDbDataReader dr;
